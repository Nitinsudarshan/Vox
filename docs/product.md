...
3. **Voice Scribble to Structured Output**: Transforms rambling audio notes into polished markdown templates (executive summaries, key decisions, action points). *(Not built: the unreachable capture mode that restructured transcripts was removed. A voice note promoted to a Scribble is enriched — title, summary, topics, entities — instead.)*
