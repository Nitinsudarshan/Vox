...
| [asr-shootout.md](asr-shootout.md) | How Vox compares recognition stacks: the corpus, the audio gate that decides whether a corpus can settle anything, the engine matrix, the one evaluator, and the taxonomy that says whether a bad result is the model, the engine, the segmenter, the prompt or the recording. |
