...
### Decision 69: Talkback Was Removed, and Decisions 47–56 Describe Code That Is Not In The Tree

- **Context**: A speech-architecture audit traced the implementation rather than the documentation and found that `native/src-tauri/src/talkback/` and `native/src-tauri/src/tts/` do not exist. Decisions 47 through 56 describe them in detail — eleven Talkback modules, `tts::TtsProvider` with Piper behind it, `tts/discovery.rs`, `tts/manifest.rs`, `tts/installer.rs`, `resources/voice-manifest.json`, `scripts/build-voice-manifest.mjs`, a bounded `SpeechPipeline`, `talkback::turn::TurnDetector`, and `docs/talkback/RESEARCH.md`. None of those paths are in the repository. What survives is residue: `LLMClient::complete_streaming` and `parse_stream_line` with no callers outside their own tests, `KanbanSource::Talkback`, a `'talkback'` member of `TodoSourceKind`, and doc-comments across `capture/stt.rs`, `capture/speech_health.rs`, `capture/device.rs`, `context/assembly.rs` and `pipeline/enrichment.rs` that reference a subsystem their reader cannot open.
- **Decision made**: Record the removal here, because this log is append-only and Decisions 47–56 cannot be edited to match. Mark the affected `maybe_later.md` items (§§1, 2, 3, 5) as pointing at absent code, correct FR-2.4's claim that "Pipeline and TTS code preserved", and rebuild `tts/` as an interface with no provider behind it rather than pretending one is there to adapt.
- **Reason**: `docs/README.md` opens by saying documentation that outlives its subject is worse than none, because the next reader treats it as current — and that is exactly what happened. A plan for this work assumed a local TTS path existed to wrap, which would have produced an adapter over nothing. The research in those decisions is still good; the claims about what exists are not, and only the second half needed correcting.
- **Alternatives considered**: deleting Decisions 47–56 (refused — the log is append-only precisely so that a reversed decision leaves a trace rather than a gap); restoring Talkback wholesale (refused — it is a product decision about whether Vox has a voice surface, not a documentation fix, and it belongs to whoever makes that decision); leaving the residue (refused — `complete_streaming` with no callers is a maintenance cost that looks like a feature).
- **Impact**: `maybe_later.md` §§1, 2, 3 and 5 each carry a note saying the code they name was removed. `docs/requirements.md` FR-2.4 no longer claims preserved TTS code. `native/src-tauri/src/tts/` is a speech-synthesis interface with a null provider, and `docs/speech-decision-log.md` D-038 records what it is for.
- **Existing Functionality Preserved**: everything, because the subsystem this describes was already gone. Nothing was removed by this decision; it records a removal that had already happened and was not written down.

### Decision 70: Rendered Markdown Is Untrusted, So The Webview Gets A CSP And Mermaid Gets Locked Down

- **Context**: A check across the speech work found a chain that predates it. `MarkdownView.tsx` initialised Mermaid with `securityLevel: 'loose'` and `htmlLabels: true`, then injected the rendered SVG with `dangerouslySetInnerHTML`, in a webview whose `tauri.conf.json` set `"csp": null`. `'loose'` is the setting that passes HTML in a diagram label straight through to the output. `MarkdownView` renders `CaptureDetailModal` — web-captured page content — and `MeetingSummaryPanel` — model output written from a transcript. Both are content Vox did not write, and `docs/capture.md` already states the principle: captured content is never an instruction. A `mermaid` block reaching either surface could execute script in the webview, with the whole Tauri command surface behind it and nothing to stop it.
- **Decision made**: three layers, none of which relies on the others. Mermaid runs at `securityLevel: 'strict'`, in a `mermaidConfig()` function a test asserts against, so the setting is reviewable rather than buried in an effect. Every SVG passes through `sanitizeSvgMarkup` at the point of injection, which removes script elements, `on*` handlers and executable URL schemes. And the webview gets a real content security policy, whose load-bearing clause is `script-src 'self'` — no `unsafe-inline`, no `unsafe-eval`.
- **Reason**: the configuration flag is one careless edit from being `'loose'` again, which is how it got there; the sanitizer is the layer that does not depend on remembering. The CSP is what makes the difference between a bug and an incident if both are ever wrong at once.
- **Verified before shipping, not assumed**: the production bundle was built and inspected. `dist/index.html` carries no inline script, and no bundle contains `eval(` or `new Function(` — so `script-src 'self'` breaks nothing that exists today. The rest of the policy is deliberately permissive where the app has a real need: `style-src 'unsafe-inline'` because Tailwind and Mermaid inject stylesheets at runtime, Google Fonts origins because `index.html` loads Inter from them, `img-src https:` because rendered markdown may reference remote images, and the Tauri `ipc:` and `asset:` origins because that is how the frontend reaches the backend and plays a recording.
- **Alternatives considered**: `securityLevel: 'sandbox'`, which renders each diagram in an iframe (refused — it disables all scripting, which is more than needed, and an iframe cannot be panned or zoomed as one element, which is the other half of this change); a general HTML sanitizer profile over the SVG (refused — the standard SVG allowlists drop `foreignObject`, which is what wraps every flowchart label, and a sanitizer that silently deletes half a diagram is reported as a rendering bug and worked around).
- **Impact**: diagrams containing HTML labels now render that HTML as text rather than as markup. That is the intended change and the only user-visible one.
- **Not verified here, and it needs to be**: a CSP takes effect in a packaged build, not against a Vite dev server, and this repository's Windows build could not be produced in the environment the change was made in. The first production build after this decision should be opened once with the developer console visible, and any `Content-Security-Policy` violation it logs is a directive to widen — not a reason to go back to `null`. Setting `"csp": null` restores the previous behaviour exactly, if it has to be done in a hurry.
- **Existing Functionality Preserved**: markdown, tables, code blocks, images and diagram rendering are unchanged. Diagram *sizing* changed for a separate reason recorded with the viewport work; the security posture did not drive it.

---

### Decision 71: Dictation Cleanup Is Opt-In, and Raw Is the Default

- **Context**: The Tier 2 rewrite (`capture::rewrite`, confined to dictation by Decision 65) had become automatic. `CleanupStyle::from_setting` read an empty or unrecognised `stt.cleanup_style` as `Faithful`, so every dictation — hotkey and pill alike — waited on a model call of up to five seconds before its text was usable, and with a cloud provider selected the dictated text left the machine without the user having asked for that. The pill's style list labelled `Faithful` as the default. The one on/off switch, `stt.text_transform` (default off), was read only by `rewrite_dictation`, one of three post-hoc select-and-replace commands (`rewrite_dictation`, `get_cleanup_target`, `apply_dictation_cleanup`) that were registered and called by nothing in the frontend. The setting that said "off by default" switched nothing off.
- **Decision made**: `Raw` is the default and asks no model. An empty or unrecognised `stt.cleanup_style` reads as `Raw`; `faithful`, `clean`, `polished` (`professional` is accepted as a synonym) and `concise` are opt-in, and each is bounded by `capture::dictation::CLEANUP_TIMEOUT` (5 s) — a timeout, a provider error or an unchanged answer all resolve to the text as transcribed, and none of them can fail the dictation. `cleanup_style` is the only control: `stt.text_transform` and the three post-hoc commands are removed. A `todo` or `scribble` capture always runs `Raw`, because what it feeds is structured by what comes next. When a style does change the text, the voice note keeps the pre-cleanup text (after the deterministic pass) as `raw_content` and names the style in `cleanup_style`, so the change can be shown as a diff.
- **Reason**: the rewrite costs a wait before the text is usable, may change words, and with a cloud provider sends the dictation off the machine. Each of those is something the user turns on, never something they discover has been happening (`settings::SttSettings::cleanup_style`). NFR-1 and NFR-3 make a local, zero-cost baseline a requirement, and a default that calls a model on every dictation contradicts both. It also bought little: a cold local model routinely takes longer than the timeout to load, so the old default could spend the full five seconds and then fall back to the raw text anyway.
- **Alternatives considered**: keeping `text_transform` beside the style list (refused — two controls for one behaviour can disagree, and `Raw` already means off); keeping `Faithful` as the default because it is the narrowest rewrite (refused — narrowness bounds what a rewrite may change, not what it costs or where the text goes); keeping the post-hoc commands for a future surface (refused — code with no caller is a maintenance cost that looks like a feature, as Decision 69 found).
- **Impact**: an existing `settings.json` whose `cleanup_style` is empty or absent now gets `Raw`, so those dictations stop calling a model; a user who picked a style keeps it. A stale `text_transform` key is ignored on load and dropped on the next save, since the settings structs have no `deny_unknown_fields` (the same path Decision 36's `show_floating_pill` took).
- **Existing Functionality Preserved**: Decision 65 stands — the rewrite is still dictation-only. The deterministic pass (normalisation, learned corrections, snippets, output script) runs on every dictation as before, a style can still be chosen from the pill or Settings, and a voice note's diff still shows what a style changed.

---

### Decision 72: The Dictation Hotkey and the Pill Share One Pipeline

- **Context**: Decisions 36 and 38 carried forward, unresolved, that the global dictation hotkey and the pill's click-to-record diverge after transcription. Both recorded through one `AudioRecorder`; each then kept its own copy of everything between the recorder and the vault — engine choice, prompt, normalisation, snippets, romanisation, cleanup — and the copies had drifted. The hotkey path never re-checked for an empty transcript after normalisation, so a recording in which Whisper heard only filler was typed into the focused field and saved as an empty voice note. The hotkey path also retried clipboard writes and waited up to fifteen seconds for focus to return by sleeping on the async runtime that drives the hotkey watchdog and every IPC call.
- **Decision made**: `capture::dictation` is the one pipeline, and `hotkeys::stop_dictation_session` and `commands::process_captured_audio` both call its three steps in order. `transcribe` decodes with Parakeet when it is selected and installed and with Whisper otherwise, primed with the dictionary and — only when `enable_initial_prompt` is on — the custom prompt; the decode runs on the blocking pool, a diagnostics snapshot is recorded either way, and a missing Whisper model is downloaded with the pill told so. `finish_text` is the deterministic pass, and returns nothing when nothing is left, which both callers treat as no speech: nothing is typed, saved or turned into a todo. `clean_up` is Decision 71. What happens to the finished text stays with each surface, because that is the part they genuinely differ in: the hotkey injects it into the field that had focus and then saves a voice note; the pill saves a voice note, whose id its result now carries, and for a `todo` capture a TODO card. Injection, clipboard writes and the focus-return wait run on the blocking pool.
- **Default injection, recorded here because no entry in this log does**: `clipboard.injection_method` defaults to `clipboard_paste` — the text goes onto the clipboard and Ctrl+V is sent through Win32 `SendInput`, after any modifier still held from the hotkey is released. `keystrokes`, per-character typing through `enigo`, is the alternative for applications that refuse a paste. Decision 13 describes `enigo` injection; it is now the fallback, not the default. With `clipboard.copy_to_clipboard` off, paste injection still overwrites the clipboard and does not restore it — `TODO(clipboard-restore)` in `hotkeys::stop_dictation_session` marks it.
- **Reason**: two copies of one pipeline drift, and these two already had — a fix made on one surface silently skipped the other. Splitting after the text is final puts the boundary exactly where the surfaces' behaviour differs, and nowhere else.
- **Alternatives considered**: sharing only the transcription step (refused — the empty-text bug lived in the step after it); moving injection into the shared pipeline (refused — the pill does not inject, and a pipeline that has to be told not to type is two pipelines again).
- **Existing Functionality Preserved**: hold-to-talk and toggle-to-talk (Decision 37), the focus guard that refuses to type into a different window or tab and leaves the text on the clipboard instead, the `had_audio` gate before transcription, and the voice-note history (Decision 38). Decision 38's deferred question — whether click-to-record should ever inject — is still open; this decision shares the pipeline, not the ending.

---

### Decision 73: Provider API Keys Live in the OS Credential Store, and the Webview Sees a Placeholder

- **Context**: LLM provider API keys — `provider.provider_keys`, one per provider slug, and the legacy single `provider.cloud_api_key` — were written into `settings.json` in plain text beside every other preference, and `get_settings` handed them to the webview whole. Decision 44 had moved the Google OAuth tokens into the OS keyring on the principle of no secrets in user-readable files; the provider keys never followed.
- **Decision made**: `AppSettings::save` writes every non-empty key to one OS credential-store entry — service `com.vox.app.providers`, account `provider_keys`, holding a JSON object keyed by provider slug, with the legacy key under `cloud_api_key` — and blanks the keys in the file; `AppSettings::load` puts them back in memory. Everything sent to the webview goes through `AppSettings::for_webview`, which replaces each stored key with the placeholder `__vox_stored_key__`. `save_settings` resolves a placeholder sent back unchanged to the key it stands for, and a placeholder with nothing behind it is saved as empty rather than as a key. A key found in an older `settings.json` wins over the store and moves into it on the next save. With no credential store available — a Linux machine with no Secret Service running, say — the keys stay in `settings.json` as before, with a warning in the log.
- **Reason**: a key at rest belongs where the OS keeps secrets, beside the OAuth tokens, not in a file every backup and sync client copies. The webview never needs a key's value, only to know that one is set, and it is the surface Decision 70 had to defend against untrusted rendered content. The fallback is deliberate: refusing to save a key the user just pasted would be worse than storing it where it always was (`providers::secrets`).
- **Alternatives considered**: refusing to save without a credential store (refused, as above); scrambling the key inside the file, as the OAuth token fallback does (refused — that fallback is now labelled obfuscation rather than encryption, because anyone with the source can reverse it).
- **Impact**: `get_settings` and the `settings-changed` event carry placeholders only. Unit tests use a store that is never available, so no test writes to a developer's or a CI runner's real credential store; the `rust-windows` CI job compiles the Windows backend, and no test exercises it against a real desktop (`docs/testing.md`).

---

### Decision 74: A Fresh Release Install Keeps Its Data in the OS App-Data Folder, and Existing Data Is Never Moved

- **Context**: Vox resolved its base directory — the vault and `config/` (settings, models) — from the process's working directory. A packaged app's working directory is whatever launched it: `System32` from autostart, the install folder from a shortcut. A fresh release install could therefore write its vault somewhere unwritable, or somewhere different on each launch. Decision 52 fixed this for the voice installation alone, and `maybe_later.md` §4 deferred the rest, because changing the base directory moves every existing user's data and that is a migration.
- **Decision made**: `lib.rs::choose_base_dir` resolves, in order: `VOX_HOME`, if set; in a debug build, an existing `.vox` (or legacy `.relay`) in the crate directory; an existing `.vox` or `.relay` beside the executable, or in the working directory or any ancestor of it (including `native/src-tauri/` and `src-tauri/` beneath each); and only when none exists, the per-user application-data folder in a release build — `%APPDATA%\com.vox.app` on Windows, `~/Library/Application Support/com.vox.app` on macOS, `$XDG_DATA_HOME/com.vox.app` or `~/.local/share/com.vox.app` elsewhere — or the crate's `.vox` in a debug build. If no application-data folder can be resolved, the old `cwd/.vox` fallback remains. The vault is `<base>/vault` unless `vault.directory` is set, and configuration is `<base>/config`.
- **Reason**: this is the part of `maybe_later.md` §4 that ships without a migration. Existing data is honoured wherever it is found, because moving someone's vault is a migration with its own consent, never a side effect of an upgrade. An existing install keeps working from exactly where it was; only an install with nothing on disk lands somewhere new, and that place is writable and stable.
- **Alternatives considered**: moving existing folders into application data automatically (refused — a data move needs consent; §4's design, offer to move and leave the old copy in place, is still the way to do it); always using application data and ignoring existing folders (refused — every upgraded user would open an empty vault).
- **Impact**: `VOX_HOME` is read under that name only; unlike the variables `env_config` reads, it has no `RELAY_` fallback. `lib.rs::base_dir_tests` pins each case: a fresh release install uses application data, data beside the executable is never moved, data in the working-directory tree is still found, `VOX_HOME` overrides everything, a debug build prefers the checkout, and with no application-data folder the old fallback applies. What remains of `maybe_later.md` §4 is the offer to move a legacy folder.

---

### Decision 75: The Trigger Engine, Its Settings Tab and the MCP Action Stubs Are Removed

- **Context**: Decision 35 deferred trigger phrases and MCP dispatch and kept the pieces "available to reconnect later": `TriggerEngine` and its `triggers.json`, the `get_triggers`/`save_triggers` commands, `TriggerSettings.tsx`, and `McpRouter::dispatch_action`. What remained was worse than absence. `TriggerSettings.tsx` was reachable again, as the `MCP Triggers` tab of Settings › Dictionary & Snippets, and saved phrases to `triggers.json` — but Decision 35 had removed the only call to `TriggerEngine::match_transcript`, so nothing ever matched a phrase. `McpRouter::dispatch_action` had no callers, and its four stubs (`execute_calendar`, `execute_local_reminder`, `execute_notion`, `execute_gdrive`) returned success — "Scheduled calendar event: '…'" — for actions that never ran, after logging the dictated text at `info`.
- **Decision made**: Remove `native/src-tauri/src/triggers/` (`TriggerEngine`, `TriggerConfig`, the `triggers.json` format), the `get_triggers`/`save_triggers` commands, `TriggerSettings.tsx` and its tab, and `McpRouter::dispatch_action` with its stubs. `McpRouter::assemble_context` and `McpRouter::execute_action` stay: they build a real context pack and run the real, confirmation-gated `ActionDispatcher` for an MCP caller, though nothing calls them yet and Vox has no MCP client.
- **Reason**: `rules/ui-components.md` — no fake controls. A settings screen that saves configuration no code path consumes is a fake control that survives a restart, and a function that reports a scheduled event that was never scheduled is the same failure one layer down. Logging what the user dictated, for a feature that did nothing, was a privacy cost with no return.
- **Alternatives considered**: hiding the tab again and keeping the engine (refused — that was Decision 35's state, and the tab came back); keeping the stubs behind a flag (refused — a stub that reports success is not a placeholder, it is a wrong answer).
- **Supersedes**: Decision 35's "available to reconnect later" for everything listed above. Decisions 8 and 10 stay superseded as Decision 35 left them, and `docs/requirements.md` marks FR-2.3 as not built.
- **Impact**: a `triggers.json` already on disk is left alone and never read. Voice triggers that act — a matcher on the capture path, an MCP client, and a confirmation step before anything runs — are a new feature needing their own decision, not a reconnection.
- **Existing Functionality Preserved**: snippets (`settings.snippets`, whose spoken `trigger` expands to text — a different feature, and one that works), the dictionary, dictation, and the universal action dispatcher.
